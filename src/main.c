/**
 * @file main.c
 * @brief Entry point of the Process Browser application.
 *
 * This file contains the main loop which orchestrates the Model (proc.c),
 * View (ui.c), and Controller logic (input handling).
 */
 ...
 *
 * Initializes the process list and UI, then enters the main event loop.
 * Handles user input, updates process data, filters, sorts, and renders the UI.
 *
...
int main() {
    ...
    // Flag to trigger the confirmation dialog overlay
    ...
    while (running) {
        // 1. Update Model (only if not searching or in a dialog to prevent UI jitter)
        if (!search_mode && !kill_confirm_mode) {
            proc_list_update(&all_processes);
        }

        // Apply Logic: Filter -> Sort
        ...
        // 2. Render View
        ui_draw(&visible_processes, selected, scroll_offset, filter, search_mode);

        // If in confirmation mode, draw the overlay dialog
        if (kill_confirm_mode && visible_processes.count > 0) {
            ui_show_confirm_dialog(visible_processes.list[selected].name,
        ...
        // --- Logic: Kill Confirmation Mode ---
        if (kill_confirm_mode) {
            if (ch == 'y' || ch == 'Y') {
                // User confirmed kill
                if (visible_processes.count > 0) {
                    proc_kill_process(visible_processes.list[selected].pid);
                }
                kill_confirm_mode = 0;
                // Reduce timeout to update list immediately
                timeout(100);
            }
            else if (ch == 'n' || ch == 'N' || ch == 27) { // ESC or N
                kill_confirm_mode = 0;
            }
        ...
        if (search_mode) {
            ...
            else if (ch >= 32 && ch <= 126 && strlen(filter) < 49) {
                int len = strlen(filter);
                filter[len] = (char)ch;
                filter[len + 1] = 0;
                selected = 0;
        ...
        switch (ch) {
            ...
            case KEY_DOWN:
                if (selected < visible_processes.count - 1) {
                    selected++;
                    if (selected >= scroll_offset + list_height) scroll_offset++;
                }
                break;
        }
    }
